...
func TestStartRunUI(t *testing.T) {
	...
	// operation
	browser.Run(t, ctx, startRunTasks(t, svc.System.Hostname(), ws.Organization.Name, ws.Name, run.PlanAndApplyOperation))

	// now destroy resources with the destroy-all operation
	browser.Run(t, ctx, chromedp.Tasks{
		// go to workspace page
		chromedp.Navigate(workspaceURL(svc.System.Hostname(), ws.Organization.Name, ws.Name)),
