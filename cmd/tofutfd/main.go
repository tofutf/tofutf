...
import (
	...
	"github.com/tofutf/tofutf/internal/fxlib"
		...
		Run: func(cmd *cobra.Command, args []string) {
			fx.New(
				...
				fx.Invoke(func(
					...
					go func() {
						err := d.Start(ctx, make(chan struct{}))
						logger.Error(err, "daemon startup has encountered an error")
						l.Shutdown()
					}()
...
func ProvideConfig() func() (daemon.Config, error) {
	return func() (daemon.Config, error) {
		...
		if err := cmdutil.SetFlagsFromEnvVariables(cmd.Flags()); err != nil {
			return cfg, errors.Wrap(err, "failed to populate config from environment vars")
		}

		if err := cmd.Execute(); err != nil {
			return cfg, errors.Wrap(err, "failed to execute command")
		}
