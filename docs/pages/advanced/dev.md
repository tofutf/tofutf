...
```
git clone git@github.com:leg100/tofutf.git
```
...
* Automatically restart `tofutfd` whenever changes are made to code.
...
A `modd.conf` is included in the tofutf repository. Once you've installed `modd`, run it from the root of the repository and it'll perform the above tasks.
